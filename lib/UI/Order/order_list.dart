...
//
//     debugPrint("Refreshing orders for type: ${widget.type}");
//     debugPrint(
//         "Table: ${widget.selectedTableName}, Waiter: ${widget.selectedWaiterName}");
//
//     context.read<OrderTodayBloc>().add(
//           OrderTodayList(yesterdayDate, todayDate,
//               widget.selectedTableName ?? "", widget.selectedWaiterName ?? ""),
//         );
//     setState(() {
//       orderLoad = true;
//     });
...
//   void initState() {
//     super.initState();
//     // Always refresh orders when the widget is initialized
...
//     // Refresh orders when filter parameters change
//     if (oldWidget.selectedTableName != widget.selectedTableName ||
//         oldWidget.selectedWaiterName != widget.selectedWaiterName) {
//       WidgetsBinding.instance.addPostFrameCallback((_) {
//         if (mounted) {
//           refreshOrders();
//           setState(() {
...
class OrderViewViewState extends State<OrderViewView> {
  ...
  void refreshOrders() {
    // This method is kept for compatibility but doesn't trigger new API calls
    // The parent handles all API calls centrally
    if (!mounted || !context.mounted) return;
    setState(() {
      selectedTableName = null;
      selectedWaiterName = null;
      selectTableValue = null;
      selectWaiterValue = null;
    });
    context.read<OrderTodayBloc>().add(
          OrderTodayList(yesterdayDate, todayDate, selectedTableName ?? "",
              selectedWaiterName ?? ""),
        );
    debugPrint("tsble:${widget.selectedTableName}");
    debugPrint("wait:${widget.selectedWaiterName}");
    debugPrint("table:${selectedTableName}");
    debugPrint("wait:${selectedWaiterName}");
    debugPrint(
  ...
  void initState() {
    super.initState();
    // Don't make API calls here, use shared data from parent
  ...
  void didUpdateWidget(OrderViewView oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Update local data when parent passes new shared data
    if (widget.sharedOrderData != null) {
      setState(() {
        getOrderListTodayModel = widget.sharedOrderData!;
      });
